...
export default function LeaderboardPage() {
    ...
    try {
      ...
      if (json.success) {
        ...
        if (currentPoppers.length > 0) {
           ...
           // Clear these specific IDs after animation duration (600ms)
           setTimeout(() => {
             setPoppingIds([]); 
           }, 600);
        }
  ...
  const cardVariants = {
    idle: { 
        scale: 1, 
        backgroundColor: "rgba(255, 255, 255, 1)", // Standard White
        borderColor: "rgba(241, 245, 249, 1)" // Slate-100
    },
    pop: { 
        scale: [1, 1.05, 1], // The POP
        backgroundColor: ["rgba(255, 255, 255, 1)", "rgba(254, 249, 195, 1)", "rgba(255, 255, 255, 1)"], // Flash Yellow-50
        borderColor: ["rgba(241, 245, 249, 1)", "rgba(250, 204, 21, 1)", "rgba(241, 245, 249, 1)"], // Flash Yellow-400
        transition: { 
            duration: 0.5, 
            ease: "easeInOut" 
        }
          ...
          <div className="overflow-y-auto custom-scrollbar p-4 space-y-3 flex-1">
            <AnimatePresence mode="popLayout">
              {kids.map((kid, index) => {
                ...
                return (
                  // 1. OUTER DIV: Handles List Position/Reordering (Layout)
                  <motion.div
                    layout
                    key={kid._id}
                    ...
                    className="relative"
                  >
                    {/* 2. INNER DIV: Handles Scale/Pop Animation ONLY */}
                    {/* IMPORTANT: Do NOT put 'layout' here. */}
                    <motion.div
